...
import { parseGetHostsResponse } from '../parse.js';
...
function parseHostsWithDebug(result, client) {
    try {
        ...
        if (process.env['NAMECHEAP_DEBUG'] === '1') {
            ...
            process.stderr.write(`[namecheap-mcp] parseGetHostsResponse failed: ${err instanceof Error ? err.message : String(err)}\n` +
                `[namecheap-mcp] last ${tail.length} bytes of raw XML:\n${tail}\n`);
        }
        throw err;
    }
}
function cleanHostsForOutput(parsed, rawHosts) {
    return {
...
export function registerDnsTools(server, getClient) {
    ...
    server.registerTool('set_dns_hosts', {
        description: 'DO NOT USE FOR SINGLE-RECORD CHANGES. Use `update_dns_record` to add, update, or delete one record safely.\n\n' +
        ...
        inputSchema: {
            ...
            }).refine((r) => r.recordType !== 'MX' || r.mxPref !== undefined, { error: 'mxPref is required for MX records', path: ['mxPref'] })).describe('Complete list of DNS records to set. Replaces all existing records.'),
            confirmReplaceAll: z.literal(true).describe('Must be exactly `true` to acknowledge this call replaces the entire zone and deletes any record not in `hosts`.'),
            expectedDeletions: z.number().int().min(0).describe('The number of currently existing records you expect this call to delete. ' +
            ...
            preserveEmailType: z.boolean().optional().describe('If true (default), reads the current EmailType from the zone and passes it through. Set to false to reset.'),
        },
        ...
        try {
            ...
            const key = (h) => `${h.hostName}\x00${h.recordType}\x00${h.address}`;
            ...
            if (actualDeletions !== expectedDeletions) {
                return {
                    ...
                    content: [{
                            type: 'text',
                            text: `Refusing to write: expectedDeletions=${expectedDeletions} but this call would actually delete ${actualDeletions} existing records. ` +
            ...
            await client.execute('namecheap.domains.dns.setHosts', { SLD: sld, TLD: tld, ...extraParams, ...hostParams }, 'POST');
        ...
        try {
            ...
            if (currentRecords.length === 0 && parsed.usingNamecheapDns && !allowEmptyBaseline) {
                return {
                    ...
                    content: [{
                            type: 'text',
                            text: `Refusing to write: getHosts returned 0 records but this zone is using Namecheap DNS. ` +
            ...
            const matches = (h) => recordMatches(h, record);
            let updatedRecords;
            if (operation === 'delete') {
            ...
            if (actualDelta !== expectedDelta) {
                return {
                    ...
                    content: [{
                            type: 'text',
                            text: `Internal merge check failed: expected delta ${expectedDelta} but got ${actualDelta}. ` +
            ...
            const writeParams = { SLD: sld, TLD: tld, ...hostParams };
            if (parsed.emailType)
                writeParams['EmailType'] = parsed.emailType;
    ...
    server.registerTool('set_email_forwarding', {
        description: 'DESTRUCTIVE: Replaces ALL email forwarding rules for a domain. Any rules not included will be deleted. ' +
        ...
        inputSchema: {
            ...
            confirmReplaceAll: z.literal(true).describe('Must be exactly `true` to acknowledge this call replaces all forwarding rules.'),
        },
    ...
    server.registerTool('list_dns_snapshots', {
        description: 'List local DNS zone snapshots for a domain. Snapshots are written automatically before every write (and on every get_dns_hosts read) to ~/.config/namecheap-mcp/snapshots/. Use this to find a snapshot to restore with restore_dns_snapshot.',
        inputSchema: { domainName: z.string().describe('The domain name, e.g. "example.com"') },
    }, async ({ domainName }) => {
        try {
    ...
    server.registerTool('restore_dns_snapshot', {
        description: 'Restore a DNS zone from a local snapshot file. DESTRUCTIVE: replaces the entire current zone with the snapshotted record set. ' +
        ...
        try {
            ...
            try {
                ...
                return {
                    ...
                    content: [{
                            type: 'text',
                            text: `Could not read snapshot at ${snapshotPath}: ${msg}. ` +
            ...
            const writeParams = { SLD: sld, TLD: tld, ...hostParams };
            if (snapshot.emailType)
                writeParams['EmailType'] = snapshot.emailType;
            await client.execute('namecheap.domains.dns.setHosts', writeParams, 'POST');
            return {
                content: [{
                        type: 'text',
